...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, dilated=True, multi_grid=True,
                 deep_base=True, norm_layer=nn.BatchNorm2d, spm_on=True):
...
class base_resnet(nn.Module):
    def __init__(self):
        ...
        self.resnet50 = ResNet(block=Bottleneck, layers=[3, 4, 6, 3])

    def forward(self, x):
        aux, x = self.resnet50(x)
        return aux, x
